describe('Coral.Select', function() {
  ...
  describe('Instantiation', function() {
    ...
    it('should be possible using new', function() {
      var select = helpers.build(new Coral.Select());
    ...
    it('should be possible to clone using markup', function() {
      const el = helpers.build(window.__html__['Coral.Select.multiple.base.html']);
      helpers.cloneComponent(el);
    });

    it('should be possible to clone using markup with framework data', function() {
      const el = helpers.build(window.__html__['Coral.Select.mustache.html']);
      helpers.cloneComponent(el);
    });

    it('should be possible to clone using js', function() {
      const el = helpers.build(new Coral.Select());
      helpers.cloneComponent(el);
    });
  });
  ...
  describe('API', function() {
    ...
    beforeEach(function(done) {
      el = helpers.build(new Coral.Select());
  ...
  describe('Markup', function() {
    ...
    describe('#selectedItem', function() {
      it('should allow selecting items in the DOM', function() {
        const el = helpers.build(window.__html__['Coral.Select.selected.html']);
    ...
    describe('#placeholder', function() {
      ...
      it('should update the label to the selected item when there is selection', function() {
        const el = helpers.build(window.__html__['Coral.Select.placeholder.selected.html']);
      ...
      it('should show the placeholder when there is selection and multiple=true', function() {
        const el = helpers.build(window.__html__['Coral.Select.placeholder.multiple.selected.html']);
        var item = el.items.getAll()[2];
        expect(el.selectedItem).to.equal(item);
        expect(el._elements.label.textContent).to.equal(el.placeholder, 'label should match the placeholder');
      });
    ...
    describe('#value', function() {
      it('should be possible to set the value using markup (in single mode)', function() {
        const el = helpers.build(window.__html__['Coral.Select.value.html']);
      ...
      it('should be possible to set the value using markup (in multi mode)', function() {
        const el = helpers.build(window.__html__['Coral.Select.multiple.value.selected.html']);
