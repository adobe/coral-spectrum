describe('Coral.Shell.Workspaces', function() {
  describe('Namespace', function() {
    it('should be defined in the Coral.Shell namespace', function() {
      expect(Coral.Shell).to.have.property('Workspaces');
      expect(Coral.Shell).to.have.property('Workspaces');
    });
  });
  ...
  describe('Instantiation', function() {
    it('should support creation from markup', function() {
      ...
      expect(el instanceof Coral.Shell.Workspaces).to.equal(true);
    });

    it('should support creation from js', function() {
      var el = helpers.build(new Coral.Shell.Workspaces());
      expect(el instanceof Coral.Shell.Workspaces).to.equal(true);
    });

    it('should create a workspaces component with predefined items', function() {
      const el = helpers.build(window.__html__['Coral.Shell.Workspaces.base.html']);
      expect(el.items.length).to.equal(3);
    });
  
    it('should be possible to clone using markup', function() {
      helpers.cloneComponent(window.__html__['Coral.Shell.Workspaces.selected.html']);
    });
  
    it('should be possible to clone using js', function() {
      const el = new Coral.Shell.Workspaces();
      el.items.add();
  ...
  describe('API', function() {
    ...
    describe('#selected', function() {
      it('should select another item', function() {
        const el = helpers.build(window.__html__['Coral.Shell.Workspaces.selected.html']);
  ...
  describe('User Interaction', function() {
    ...
    beforeEach(function() {
      const el = helpers.build(window.__html__['Coral.Shell.Workspaces.focus.html']);
