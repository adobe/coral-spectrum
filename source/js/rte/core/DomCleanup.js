 ...
 * the component's {@link CQ.form.rte.HtmlSerializer}, which applies additional rules to
 * cleanup the generated HTML).
 * @constructor
 * Creates a new DomCleanup.
 * @param {Object} config The kernel's configuration
 */
CQ.form.rte.DomCleanup = new Class({
     ...
     * used. Format is defined at {@link CQ.form.rte.plugins.EditToolsPlugin#htmlPasteRules}
     * @private
     * @type Object
     */
    pasteRules: null,
    ...
    markForRemoval: function(dom) {
        if (!CQ.form.rte.Common.arrayContains(this.elementsToRemove, dom)) {
    ...
    isPreProcessing: function() {
        var dcu = CQ.form.rte.DomCleanup;
                ...
                if (com.isTag(lastTextChild, "br")) {
                    ...
                    if (com.isTag(predesc, "br")) {
                        if (com.ua.isGecko || com.ua.isWebKit) {
                            ...
                            CQ.form.rte.DomProcessor.fixEmptyLinefeedIE(context, predesc);
                        }
                    } else {
                        lastTextChild.parentNode.removeChild(lastTextChild);
                    }
    ...
    handleAnchor: function(dom) {
        ...
        if (com.isTag(dom, "a")) {
            ...
            var hrefAttrib = CQ.form.rte.HtmlRules.Links.getLinkHref(dom);
            if (nameAttrib) {
                if (this.isPreProcessing()) {
                    ...
                    if (!com.ua.isWebKit) {
                        ...
                        com.setAttribute(imgReplacement, "src",
                                CQ.form.rte.Utils.BLANK_IMAGE_URL);
    ...
    handleImage: function(dom) {
        ...
        if ((this.processingMode == dcu.PRE) && com.isTag(dom, "img")) {
            srcAttrib = com.getAttribute(dom, CQ.form.rte.Common.SRC_ATTRIB);
        ...
        if (this.processingMode == dcu.PASTE_PREPARE) {
            if (com.isTag(dom, "img")) {
                ...
                if (srcAttrib) {
                    var helperAttrib = com.getAttribute(dom, CQ.form.rte.Common.SRC_ATTRIB);
                    if (!helperAttrib) {
                        com.setAttribute(dom, CQ.form.rte.Common.SRC_ATTRIB, srcAttrib);
                    }
                }
            }
        }
     ...
     * @param {CQ.form.rte.EditorKernel} editorKernel The editor representation
    ...
    preprocess: function(editorKernel, rootDom) {
        this.processingMode = CQ.form.rte.DomCleanup.PRE;
    ...
    postprocess: function(editorKernel, rootDom) {
        this.processingMode = CQ.form.rte.DomCleanup.POST;
        this.execute(editorKernel, rootDom);
    },
    ...
    prepareHtmlPaste: function(editorKernel, rootDom, pasteRules) {
        this.processingMode = CQ.form.rte.DomCleanup.PASTE_PREPARE;
