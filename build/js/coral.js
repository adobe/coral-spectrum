  ...
  CUI.Autocomplete = new Class(/** @lends CUI.Autocomplete# */{
    ...
    _setForceSelection: function () {
      ...
      if (this.options.forceselection && !this.options.multiple) {
        ...
        if (!this._valueInput) {
          var inputName = this._input.prop('name');
          if (inputName) {
            this._valueInput = $('<input type="hidden">')
              .prop('name', inputName)
    ...
    _handleSelected: function (event) {
      ...
      if (this.options.multiple) {
        ...
        if (this.options.forceselection) {
          ...
          this._input.val(this._lastSelected); // Use _lastSelected to follow <datalist> behaviour
        }
