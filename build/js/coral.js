    ...
    _buildHandles: function () {
      ...
      that.$inputs.each(function (index) {
        ...
        if (that.options.tooltips) {
          ...
          var tooltip = $("<output>" + $(this).attr('value') + "</output>").addClass('coral-Tooltip coral-Tooltip--info').addClass(that.isVertical ? 'coral-Tooltip--positionRight' : 'coral-Tooltip--positionTop')
...
(function ($) {
  CUI.Accordion = new Class(/** @lends CUI.Accordion# */{
    ...
    construct: function (options) {
      ...
      if (this.isAccordion) this.$element.addClass("coral-Accordion");
    ...
    _setListeners: function () {
      var selector = this.isAccordion ? '> .coral-Collapsible > .coral-Accordion-itemHeader' : '> .coral-Collapsible-itemHeader';
      ...
      var selectstartSelector = this.isAccordion ? '.coral-Accordion-itemHeader' : '.coral-Collapsible-itemHeader';
    ...
    _toggle: function (event) {
      ...
      var el = $(event.target).closest(".coral-Collapsible"),
    ...
    _initElement: function (element, collapse) {
      ...
      $(element).addClass("coral-Collapsible");

      var head = this._getHeaderElement($(element)),
    ...
    _collapse: function (el) {
      ...
      el.animate({height: this._getHeaderElement(el).height()}, {
        duration: "fast",
        complete: function () {
      ...
      var head = this._getHeaderElement(el),
    ...
    _expand: function (el) {
      ...
      this._getHeaderElement(el).addClass("is-active");
    ...
    _getHeaderElement: function (el) {
      var selector = this.isAccordion ? '> .coral-Accordion-itemHeader' : '> .coral-Collapsible-itemHeader';
      return el.find(selector);
    },
    /** @ignore */
    _getHeaderFirstElement: function (el) {
      var selector = this.isAccordion ? '> .coral-Accordion-itemHeader:first' : '> .coral-Collapsible-itemHeader:first';
      return el.find(selector);
    },           
    /** @ignore */
    _prependHeaderElement: function (el) {
      var className = this.isAccordion ? 'coral-Accordion-itemHeader' : 'coral-Collapsible-itemHeader';
      el.prepend("<h3 class=\"" + className + "\">&nbsp;</h3>");
    },
    /** @ignore */
    _getHeaderIconElement: function (el) {
      var selector = this.isAccordion ? '> .coral-Accordion-itemHeader > i' : '> .coral-Collapsible-itemHeader > i';
      return el.find(selector);
    },    
    /** @ignore */
    ...
    _makeAccessible: function () {
      ...
      if (this.isAccordion) {
        ...
        this.$element.children(".coral-Collapsible").each(function (i, e) {
          var section = $(e),
            head = section.find("> .coral-Accordion-itemHeader:first"),
        ...
        head = this._getHeaderFirstElement(section);
        isActive = section.hasClass("is-active");
      ...
      this.$element.on('focusout.accordion', '.coral-Accordion-itemHeader:focusable', this._onFocusOut.bind(this));

      this.$element.on('touchstart.accordion, mousedown.accordion', '.coral-Accordion-itemHeader:focusable', this._onMouseDown.bind(this));
    },
    ...
    _onKeyDown: function (event) {
      var el = $(event.currentTarget).closest(".coral-Collapsible"),
        head = this._getHeaderFirstElement(el),
        ...
        case 38: //up arrow
          if ((isHead && this.isAccordion) || (event.which === 33 && (event.metaKey || event.ctrlKey))) {
            ...
            if (this._getHeaderFirstElement(el.prev()).focus().length === 0) {
              this.$element.find("> .coral-Collapsible:last > .coral-Accordion-itemHeader:first").focus();
            }
        ...
        case 40: //down arrow
          if (isHead && this.isAccordion) {
            ...
            if (this._getHeaderFirstElement(el.next()).focus().length === 0) {
               this.$element.find("> .coral-Collapsible:first > coral-Accordion-itemHeader:first").focus();
            }
    ...
    _onFocusIn: function (event) {
      ...
      if (isHead) {
        if (this.isAccordion) {
          this.$element.find("> .coral-Collapsible > .coral-Accordion-itemHeader[role=tab]").attr('tabindex', -1);
        }
