...
(function() {
  ...
  function ResizeEventTrigger() {
    ...
    this.nativeResizeSupport = document.attachEvent || isIE;
  }
  ...
  ResizeEventTrigger.prototype._addTriggerElement = function(element, listenerFunction) {
    ...
    obj.setAttribute('style', 'display: block; position: absolute; top: 0; left: 0; height: 100%; width: 100%; overflow: hidden; pointer-events: none; z-index: -100;');
  ...
  Coral.register = function(options) {
    ...
    if (options.baseTagName) {
      ...
      while (curBaseProto = protoChain.shift()) {
        for (var prop in curBaseProto._methods) {
          proto[prop] = curBaseProto[prop];
        }
      }
...
(function() {
  ...
  */
  var closable = {
    ...
    ON: 'on',
    /** Do not show a close button. Elements with the <code>coral-close</code> attributes will still close the popover. */
    OFF: 'off'
  };
  ...
  for (var i1 = 0, ni1 = iterated_1.length; i1 < ni1; i1++) {
    ...
    for (var i2 = 0, ni2 = iterated_2.length; i2 < ni2; i2++) {
      ...
      data = data_2;
            ...
            // @TODO: this should be a localized string for including the current // state in the title for accessibility data.todayString = data.isToday ? 'Today, ' : ''; data.selectedString = data.ariaSelected ? ' selected' : '';
...
/**/) {
  ...
  el7.className += " coral3-Select-select";
  ...
  Coral.register( /* @lends Coral.QuickActions# */ {
    ...
    */
    ...
    events: {
      ...
      'key:pageup > .coral-QuickActinons-button': '_onButtonKeypressPrevious',
      ...
      // SelectList
      'coral-selectlist:change [handle="selectList"]': '_onSelectListChange',
      ...
      */
      'items': {
        get: function() {
          // Construct the collection on first request
          if (!this._items) {
            this._items = new Coral.QuickActions.Collection({
    ...
    attachedCallback: function() {
      ...
      Coral.commons.ready(self, function() {
        // Do an initial layout when sub-components are ready and the button has had time to render (snap from
        // with-label to without-label state). Otherwise the layout calculation is inaccurate.
        Coral.commons.nextFrame(function() {
          self._layout();
        });
      });
    ...
    */
    ...
    _attachItem: function(item) {
      ...
      // SelectList Item
      var selectListItem = item._elements.selectListItem || new Coral.SelectList.Item();
      var selectListItemParent = this._elements.selectList;

      // 'insertBefore' with an undefined "before" argument fails on IE
      selectListItemParent.insertBefore(selectListItem, selectListItemParent.children[index] || null);
      selectListItem.content.innerHTML = itemData.content;

      if (itemData.icon) {
        // Cache the icon for updates
        selectListItem._icon = this._createIconElement(itemData.icon);

        // 'insertBefore' with an undefined "before" argument fails on IE
        selectListItem.insertBefore(selectListItem._icon, selectListItem.firstChild || null);
      }

      item._elements.button = button;
      item._elements.selectListItem = selectListItem;
      selectListItem._elements.quickActionsItem = item;
    ...
    _detachItem: function(item) {
      // Remove the associated Button and SelectList elements
      ...
      if (item._elements.selectListItem) {
        item._elements.selectListItem.remove();
        item._elements.selectListItem._elements.quickActionsItem = null;
      }
    ...
    /**
      Creates an Icon element from its name.

      @param {String} name
        The name of the icon. See {@link Coral.Icon} for valid icon names.
      @returns {HTMLElement|undefined}
        The HTMLElement that represents the icon.
      @ignore
    */
    _createIconElement: function(name) {
      // All icons are small
      return new Coral.Icon().set({
        icon: name,
    ...
    */
    _layout: function() {
      ...
      var selectListItems = this._elements.selectList.items.getAll();
      ...
      // Show all Buttons and SelectList Items
      for (var i = 0; i < buttons.length; i++) {
        this._toggleTabbable(buttons[i], false);
        buttons[i].show();
        selectListItems[i].show();
      }
      ...
      if (collapse) {
        if (totalButtons > 0) {
          ...
          // Hide the selectList items
          for (var k = 0; k < totalButtons - 1; k++) {
            selectListItems[k].hide();
          }
    ...
    */
    ...
    /** @ignore */
    _setSelectListHeight: function() {
      // Set height of SelectList
      this._elements.selectList.style.height = '';
    ...
    _onEscapeKeypress: function(event) {
      if (this.open && this._isTopOverlay() && this.interaction === interaction.ON) {
        this.hide();
      }
    },
    ...
    _onOverlayBeforeOpen: function(event) {
      ...
      else if (event.target === this._elements.popover) {
        // Don't allow internal Overlay events to escape QuickActions
        event.stopImmediatePropagation();
        this._setSelectListHeight();
      }
    },
    ...
    _onOverlayOpen: function(event) {
      ...
      if (event.target === this._elements.popover) {
        ...
        event.stopImmediatePropagation();
        Coral.commons.nextFrame(function() {
          self._elements.selectList.focus();
        });
      }
    },
    ...
    _onOverlayClose: function(event) {
      ...
      if (event.target === self) {
        self._elements.popover.open = false;
      ...
      else if (event.target === self._elements.popover) {
        // Don't allow internal Overlay events to escape QuickActions
        event.stopImmediatePropagation();
      }
    },
    ...
    _onSelectListChange: function(event) {
      ...
      // We wait a frame here, otherwise the Coral.SelectList private _oldSelection variable is not updated from the
      // initial selection and we cannot accurately measure if the selection changed or not
      // Note: If there is no change in selection via the property Coral.SelectList does not re-throw a change event.
      Coral.commons.nextFrame(function() {
        selectListItem.selected = false;
      });

      var item = selectListItem._elements.quickActionsItem;
      self._proxyClick(item);
    },
    ...
    _updateItem: function(item) {
      ...
      if (item._elements.selectListItem) {
        var selectListItem = item._elements.selectListItem;
        var iconElement = null;

        selectListItem.content.innerHTML = itemData.content;

        if (itemData.icon) {
          iconElement = this._createIconElement(itemData.icon);
          selectListItem._icon = iconElement;
        }
        else if (selectListItem._icon) {
          // Add the current icon back, it didn't update
          iconElement = selectListItem._icon;
        }

        if (iconElement) {
          // 'insertBefore' with an undefined "before" argument fails on IE
          selectListItem.insertBefore(iconElement, selectListItem.firstChild || null);
        }
      }
    },
    ...
    _render: function() {
      ...
      this._elements.popover.content.appendChild(Coral.templates.QuickActions.popovercontent.call(this._elements));
...
(function() {
  ...
  /**
    The Collection of Items used by the QuickActions.
    ...
    @ignore
  */
  var QuickActionsCollection = function(options) {
    Coral.Collection.call(this, arguments);

    this._host = options.host;
  };

  QuickActionsCollection.prototype = Object.create(Coral.Collection.prototype);

  QuickActionsCollection.prototype.add = function(item, insertBefore) {
    if (!(item instanceof HTMLElement)) {
  ...
  QuickActionsCollection.prototype.getAll = function() {
    return Array.prototype.slice.call(this._host.querySelectorAll('coral-quickactions-item'));
  };

  // Make available to Coral.QuickActions
  Coral.QuickActions.Collection = QuickActionsCollection;
})();
...
/**/) {
  ...
  var el1 = document.createTextNode("\n");
  frag.appendChild(el1);
  var el2 = this["popover"] = document.createElement("coral-popover");
  el2.className += " coral-QuickActions-popover";
  el2.setAttribute("placement", "bottom");
  el2.setAttribute("target", "_prev");
  el2.setAttribute("handle", "popover");
  frag.appendChild(el2);
  return frag;
});

this["Coral"]["templates"]["QuickActions"]["popovercontent"] = (function anonymous(data_0
/**/) {
  var data = data_0;
  var el0 = this["selectList"] = document.createElement("coral-selectlist");
  el0.id = Coral["commons"]["getUID"]();
  el0.className += " coral-QuickActions-selectList";
  el0.setAttribute("handle", "selectList");
  return el0;
});
/*
 * ADOBE CONFIDENTIAL
