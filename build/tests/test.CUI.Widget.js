describe('CUI.Widget', function () {
  ...
  describe('options', function () {
    var html = '<div data-option6="6"/>',
        div = $(html),
        widget = new CUI.Widget({
          element: div,
          option1: 1,
          option2: 2,
          option3: 3,
    ...
    it('should forward DOM declared data-* attributes correctly (jQuery constructed)', function () {
    // Plug the base class like subclasses get plugged:
      CUI.util.plugClass(CUI.Widget);

      // Construct via jQuery:
      var widget = $(html)
        .widget()
        .data('widget');

      expect(widget.get('option6')).to.equal(6);

      // Unplug:
      delete $.fn['widget'];
    });

    it('should forward DOM declared data-* attributes correctly ("new" constructed)', function () {
      // Parallel to how jQuery constructed widgets port their attributes to 'options', so should 'new'
      // constructed widgets:
      expect(widget.get('option6')).to.equal(6);
    });
  });
