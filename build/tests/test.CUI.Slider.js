...
;(function () {
  ...
  var SLIDER_LABELED = {
    "name" : 'slider labeled',
    "html" : '' +
      '<div class="coral-Slider coral-Slider--ticked" data-init="labeled-slider">' +
      '<label>' +
      'Basic Horizontal Slider<br>' +
      '<input type="range" min="' + SLIDER_OPTIONS_MINIMAL.min + '" max="' + SLIDER_OPTIONS_MINIMAL.max +
      '" step="' + SLIDER_OPTIONS_MINIMAL.step + '" value="' + SLIDER_OPTIONS_MINIMAL.value + '">' +
  ...
  var TEST_ATTRIBUTES = {
    "classes" : {
      "modifiers" : {
        ...
        "tooltip_info"      : "coral-Tooltip--info"
      },
      "components" : {
    ...
    describe('accessibility', function() {
      ...
      it('moves the value to the next step using keys', function() {
        ...
        $.each(keysNext, function(i, keycode) {
          ...
          $handle.trigger(e);
          currVal = $input.val();
          expect(parseInt(currVal, 10) === (parseInt(prevVal, 10) + step)).to.be.true;
        });
      });

      it('moves the value to the previous step using keys', function() {
      ...
      it('pages down on "page down" keypress', function() {
        var prevVal, currVal, page = 10,
          e = $.Event('keydown'),
          $handle = $sliderAccessibility.find('.' + TEST_ATTRIBUTES.classes.components.handle),
          $input = $sliderAccessibility.find('input');
        ...
        prevVal = parseInt($input.val(), 10);
        e.keyCode = keyPageDown;
        $handle.trigger(e);
        currVal = parseInt($input.val(), 10);
        expect(currVal === (prevVal - page)).to.be.true;
      });

      it('moved to min on "home" keypress', function() {
        var e = $.Event('keydown'),
            $handle = $sliderAccessibility.find('.' + TEST_ATTRIBUTES.classes.components.handle),
            $input = $sliderAccessibility.find('input');
      ...
      it('moved to max on "end" keypress', function() {
        var e = $.Event('keydown'),
            $handle = $sliderAccessibility.find('.' + TEST_ATTRIBUTES.classes.components.handle),
            $input = $sliderAccessibility.find('input');
  ...
  describe('CUI.LabeledSlider', function() {
    ...
    describe('with class modifier alternatingLabels="true"', function() {
      ...
      it('should have an "alternating" property set', function() {
        ...
        expect($sliderLabeledAlternatingLabels.data('labeledSlider')).to.have.property("alternating");
