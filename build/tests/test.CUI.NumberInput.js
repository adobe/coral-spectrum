...
describe('CUI.NumberInput', function() {
   ...
   describe("API", function() {
     ...
     var html = '' +
         ...
         '    <button type="button" class="js-coral-NumberInput-decrementButton coral-Button coral-Button--square" title="Decrement" >' +
         ...
         '  <span class="coral-InputGroup-button">' +
         '    <button type="button" class="js-coral-NumberInput-incrementButton coral-Button coral-Button--square" title="Increment">' +
   ...
   describe('accessibility', function() {
     ...
     var element = new CUI.NumberInput({element: $(html), min:0, max:10 });

     it('#role=spinbutton, aria-valuemin and aria-valuemax gets set when a range is provided', function() {
       if (element.$input.attr('type') === 'text') {
         expect(element.$input.attr('role')).to.equal('spinbutton');
         expect(element.$input.attr('aria-valuemin')).to.equal('0');
         expect(element.$input.attr('aria-valuemax')).to.equal('10');
         element.setValue(5);
         expect(element.$input.attr('aria-valuenow')).to.equal('5');
         expect(element.$input.attr('aria-valuetext')).to.equal('5');
         element.increment();
         expect(element.$input.attr('aria-valuenow')).to.equal('6');
         expect(element.$input.attr('aria-valuetext')).to.equal('6');
         element.decrement();
         expect(element.$input.attr('aria-valuenow')).to.equal('5');
         expect(element.$input.attr('aria-valuetext')).to.equal('5');
       } else {
         expect(element.$input.attr('min')).to.equal('0');
         expect(element.$input.attr('max')).to.equal('10');
         element.setValue(5);
         expect(element.$input.val()).to.equal('5');
         expect(element.$input.attr('aria-valuetext')).to.equal('5');
         element.increment();
         expect(element.$input.val()).to.equal('6');
         expect(element.$input.attr('aria-valuetext')).to.equal('6');
         element.decrement();
         expect(element.$input.val()).to.equal('5');
         expect(element.$input.attr('aria-valuetext')).to.equal('5');
     ...
     it('#aria-valuemin and aria-valuemax get updated when set using JS API', function() {
       element.setMax(12);
       element.setMin(2);
       if (element.$input.attr('type') === 'text') {
         expect(element.$input.attr('aria-valuemin')).to.equal('2');
         expect(element.$input.attr('aria-valuemax')).to.equal('12');
       } else {
         expect(element.$input.prop('min')).to.equal('2');
         expect(element.$input.prop('max')).to.equal('12');
       }
     });
  });
  ...
  describe("from markup", function() {
      ...
      var defaultMarkup = '' +
        ...
        it('should use the default value when the input has not value', function() {
            var nullableMarkup = '' +
              '<div class="coral-InputGroup" data-init="numberinput" data-default-value="-1">' +
              '  <span class="coral-InputGroup-button">' +
              '    <button type="button" class="js-coral-NumberInput-decrementButton coral-Button coral-Button--square" title="Decrement" >' +
              '      <i class="coral-Icon coral-Icon--sizeS coral-Icon--minus"></i>' +
              '    </button>' +
              ...
              '  </span>' +
              '</div>';
            var element = buildElement(nullableMarkup);
            var input = element.find('input');
            expect(input.val()).to.equal('-1');
            expect(element.data('numberInput').getValue()).to.equal(-1);
        });

        it('allows empty string as a default value', function() {
            var nullableMarkup = '' +
              '<div class="coral-InputGroup" data-init="numberinput" data-default-value="">' +
              '  <span class="coral-InputGroup-button">' +
              '    <button type="button" class="js-coral-NumberInput-decrementButton coral-Button coral-Button--square" title="Decrement" >' +
              '      <i class="coral-Icon coral-Icon--sizeS coral-Icon--minus"></i>' +
              '    </button>' +
            ...
            expect(element.data('numberInput').getValue()).to.equal('');
        });

        it('adds 0 as the initial value', function() {
            var nullableMarkup = '' +
              '<div class="coral-InputGroup" data-init="numberinput">' +
              '  <span class="coral-InputGroup-button">' +
              '    <button type="button" class="js-coral-NumberInput-decrementButton coral-Button coral-Button--square" title="Decrement" >' +
            ...
            expect(element.data('numberInput').getValue()).to.equal(0);
        });

        it('should not use the default value when the initial value is 0', function() {
            var nullableMarkup = '' +
              '<div class="coral-InputGroup" data-init="numberinput" data-default-value="-1">' +
              '  <span class="coral-InputGroup-button">' +
              '    <button type="button" class="js-coral-NumberInput-decrementButton coral-Button coral-Button--square" title="Decrement" >' +
              ...
              '    </button>' +
              '  </span>' +
              '  <input type="text" class="js-coral-NumberInput-input coral-InputGroup-input coral-Textfield" value="0">' +
              '  <span class="coral-InputGroup-button">' +
              '    <button type="button" class="js-coral-NumberInput-incrementButton coral-Button coral-Button--square" title="Increment">' +
              '      <i class="coral-Icon coral-Icon--sizeS coral-Icon--add" ></i>' +
              '    </button>' +
