...
describe('Coral.keys', function() {
  ...
  it('should support non-alphanumeric keys', function() {
    ...
    for (character in keys) {
      ...
      helpers.keypress(keys[character]);
    }
  ...
  it('should not trigger handlers when an event originates from an input', function() {
    ...
    // Snippets hardcoded until CUI-2700 closed
    // var snippet = window.__html__['tests/snippets/Coral.keys.inputs.html'];

    // Add HTML to body
    var snippet = '<input type="text" id="input_text">' +
      '<textarea id="textarea"></textarea>' +
      '<select id="select"></select>' +
      '<div id="editable" contenteditable="true"><strong>Text!</strong></div>';
    var $snippet = $(snippet);
    $(document.body).append($snippet);

    // Trigger key events
    helpers.keypress('a', document.querySelector('#input_text'));
    expect(spy.callCount).to.equal(0, 'Call count after A keypress in input');

    helpers.keypress('a', document.querySelector('#textarea'));
    expect(spy.callCount).to.equal(0, 'Call count after A keypress in textarea');

    helpers.keypress('a', document.querySelector('#select'));
    expect(spy.callCount).to.equal(0, 'Call count after A keypress in select');

    helpers.keypress('a', document.querySelector('#editable'));
  ...
  it('should support a map of keyCombos to handlers with delegation', function() {
    // Snippets hardcoded until CUI-2700 closed
    // var snippet = window.__html__['tests/snippets/Coral.keys.keyComboMapWithDelegation.html'];

    // Add HTML to body
    var snippet = '<div><div id="someDiv"></div></div>';
    var $snippet = $(snippet);
    $(document.body).append($snippet);
    ...
    var keys = new Coral.Keys($snippet.get(0));
  ...
  it('should support custom filter functions', function() {
    // Snippets hardcoded until CUI-2700 closed
    // var snippet = window.__html__['tests/snippets/Coral.keys.filter.html'];

    // Add HTML to body
    var snippet = '<div><div id="someDiv"></div><span id="someSpan"></span></div>';
    var $snippet = $(snippet);
    $(document.body).append($snippet);
    ...
    helpers.keypress('a', $snippet.find('#someSpan').get(0));
  ...
  it('should support event delegation', function() {
    ...
    // Add HTML to body
    var snippet = window.__html__['Coral.keys.delegation.html'];
    var $snippet = $(snippet);
    $(document.body).append($snippet);
    ...
    helpers.keypress('a', document.querySelector('#someOtherDiv'));
    expect(spy.callCount).to.equal(0);

    // Trigger a key event on the target div
    helpers.keypress('a', document.querySelector('#delegateDiv'));
    expect(spy.callCount).to.equal(1);
  ...
  it('should set event.matchedTarget correctly', function() {
    ...
    expect(spy.callCount).to.equal(1);
    expect(matchedTarget).to.equal(document.querySelector('#delegateDiv'));
  ...
  it('should support event data with delegation', function() {
    // Snippets hardcoded until CUI-2700 closed
    // var snippet = window.__html__['tests/snippets/Coral.keys.dataWithDelegation.html'];
  ...
  it('should scope events to a given element', function() {
    // Snippets hardcoded until CUI-2700 closed
    // var snippet = window.__html__['tests/snippets/Coral.keys.scoped.html'];

    // Add HTML to body
    var snippet = '<div><div></div></div>';
    var $snippet = $(snippet);
    $(document.body).append($snippet);
    ...
    helpers.keypress('a', $snippet.children().first().get(0));
  ...
  it('should allow event delegation to immediate children', function() {
    ...
    // Add HTML to body
    var snippet = '<div><div id="outer"><div id="inner"></div></div></div>';
    var $snippet = $(snippet);
    $(document.body).append($snippet);
    ...
    helpers.keypress('a', $snippet.find('#inner').get(0));
    expect(spy.callCount).to.equal(0);

    // Trigger a key event on the inner child
    helpers.keypress('a', $snippet.find('#outer').get(0));
    expect(spy.callCount).to.equal(1);
  ...
  describe('should support keys property for Coral.Component / Coral.register', function() {
    it('should support keyboard event listeners', function() {
      ...
      keyboarder.$.appendTo(document.body);
