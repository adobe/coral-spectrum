describe('CUI.CycleButton', function() {
  ...
  describe("Interaction", function () {
    ...
    it("should focus newly visible element", function (done) {
      var element = buildElement();
      element.appendTo("#fixtures");
  ...
  describe("Event handling", function () {
    describe.skip("Simple event handlers on buttons", function () {
      // Fails with current implementation, since only events that bubbled up to
      // the container are stopped. To cover this use case, the event handler in
      // the implementation should use somethings like
      //
      //   this.$element.find(".coral-CycleButton-button").on("click", function (e) {...
      it("should abort the click event on current button and trigger it on the next", function (done) {
        var element = buildElement();

        var current = element.find(".coral-CycleButton-button").eq(0),
            next    = element.find(".coral-CycleButton-button").eq(1);
    ...
    describe.skip("Delegated event handlers for buttons on container", function () {
      // Fails with current implementation, since only events that bubbled up to
      // the container are stopped. To cover this use case, the event handler in
      // the implementation should use somethings like
      //
    ...
    describe.skip("Simple event handlers on container", function () {
      // Fails with current implementation, since stopPropagation() only stops
      // the bubbling, but consequtive handlers on the same element are still
      // executed. To cover this use case, stopImmediatePropagation() should be
      // used.
      it("should abort the click event for the current button and trigger it for the next", function (done) {
        var element = buildElement();
  ...
  describe("Single element buttons", function () {
    ...
    it.skip("should propagate the click event", function (done) {
      var element = buildSingleButtonElement();
