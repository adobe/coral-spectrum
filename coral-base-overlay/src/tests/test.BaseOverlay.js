...
describe('BaseOverlay', function() {
  ...
  // Non Modal overlay
  class OverlayDummy1 extends BaseOverlay(BaseComponent(HTMLElement)) {
    constructor() {
      super();
  ...
  describe('API', function() {
    ...
    describe('Testing getters', function() {
      it('should return element to focus when accessed using getters', function() {
        ...
        helpers.target.appendChild(button2);
        overlay.returnFocusTo(button1);
        overlay._elementToFocusWhenHidden = button2;
        expect(overlay.elementToFocusWhenHidden).to.equal(button2);
        expect(overlay.returnFocusToElement).to.equal(button1);
      });
    });
  });
