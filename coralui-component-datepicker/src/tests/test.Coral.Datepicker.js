...
describe('Coral.Datepicker', function() {
  ...
  describe('Instantiation', function() {
    it('should be possible to clone the element using markup', function() {
      helpers.cloneComponent(window.__html__['Coral.Datepicker.base.html']);
    });

    it('should be possible to clone the element using markup type="time"', function() {
      helpers.cloneComponent(window.__html__['Coral.Datepicker.type.time.html']);
    });
  });
  ...
  for (let useMomentJS = 0; useMomentJS < 2; useMomentJS++) {
    ...
    describe('API', function() {
      ...
      beforeEach(function() {
        el = helpers.build(new Coral.Datepicker());
        el._renderCalendar();
      });
      ...
      describe('#type', function() {
        it('should default to false', function() {
          expect(el.type).to.equal(Coral.Datepicker.type.DATE);
          expect(el.getAttribute('type')).to.equal(Coral.Datepicker.type.DATE);
        });
        ...
        it('should set the new type', function() {
          ...
          expect(el.getAttribute('type')).to.equal(Coral.Datepicker.type.TIME);
        });
      });
    ...
    describe('Markup', function() {
      ...
      describe('#invalid', function() {
        it('should consider the markup as the truth until user interaction happened', function() {
          helpers.build(window.__html__['Coral.Datepicker.invalid.html']);
      ...
      describe('#type', function() {
        it('should default to "date"', function() {
          const el = helpers.build(window.__html__['Coral.Datepicker.base.html']);
        ...
        it('should accept "datetime"', function() {
          const el = helpers.build(window.__html__['Coral.Datepicker.type.datetime.html']);
        ...
        it('should accept custom value format', function() {
          const el = helpers.build(window.__html__['Coral.Datepicker.type.customvalueformat.html']);
          expect(el.type).to.equal('time');
        ...
        it('should accept custom display format', function() {
          const el = helpers.build(window.__html__['Coral.Datepicker.type.customdisplayformat.html']);
          expect(el.type).to.equal('time');
          // should use the format from the markup
          expect(el.valueFormat).to.equal('HH:mm');
          // should use the default display that matches the type
      ...
      describe('#value', function() {
        ...
        it('should accept valid dates', function() {
          const el = helpers.build(window.__html__['Coral.Datepicker.value.html']);
          expect(el.value).to.equal('2000-12-31');
        ...
        it('should accept "today" as a value', function() {
          const el = helpers.build(window.__html__['Coral.Datepicker.value.today.html']);
        ...
        it('should reject invalid value strings', function() {
          const el = helpers.build(window.__html__['Coral.Datepicker.value.invalid.html']);
          expect(el.value).to.equal('');
      ...
      describe('#valueAsDate', function() {
        ...
        it('should not accept valueAsDate as an attribute', function() {
          const el = helpers.build(window.__html__['Coral.Datepicker.valueasdate.html']);
          expect(el.valueAsDate).to.be.null;
        });
      });
    });
    ...
    describe('User Interaction', function() {
      ...
      it('should not toggle the calendar when readonly', function() {
        const el = helpers.build(window.__html__['Coral.Datepicker.readonly.html']);
        // opens the popover
        el._elements.toggle.click();
