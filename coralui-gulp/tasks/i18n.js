module.exports = function(gulp) {
  ...
  gulp.task('download-localized-strings', function(cb) {
    ...
    if (!componentName) {
      ...
      grunt.fail.warn('[i18n] download-localized-strings: strings should only be retrieved for individual components.');
    }
    else {
  ...
  gulp.task('extract-localizable-strings', function(cb) {
    ...
    if (!componentName) {
      grunt.fail.warn('[i18n] extract-localizable-strings: Component name not found in package.json.');
    }
    else if (componentName === 'coralui') {
      grunt.fail.warn('[i18n] extract-localizable-strings: strings should only be retrieved for individual components.');
    }
    else {
