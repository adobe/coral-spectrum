describe('Coral.Masonry.Layout', function() {
  ...
  describe('Instantiation', function() {
    ...
    it('should be possible using markup', function() {
      const el = helpers.build(window.__html__['Coral.Masonry.variable.empty.html']);
      testInstance(el, 'variable');
    });
    
    it('should be possible to clone via markup', function() {
      helpers.cloneComponent(window.__html__['Coral.Masonry.base.html']);
    });
  });
  ...
  describe('API', function() {
    ...
    describe('#layout', function() {
      ...
      it('should have 4 default layouts defined', function() {
        for (let key in Coral.Masonry.layouts) {
          const layoutName = Coral.Masonry.layouts[key];
          const layout = new Coral.Masonry._layouts[layoutName](el);
      
          expect(layout.name).to.equal(layoutName);
          expect(layout).to.be.an.instanceof(Coral.Masonry.Layout);
        }
      });
    
      it('should be set and reflected', function() {
        el.layout = Coral.Masonry.layouts.FIXED_SPREAD;
      
        expect(el.layout).to.equal(Coral.Masonry.layouts.FIXED_SPREAD);
        expect(el.getAttribute('layout')).to.equal(Coral.Masonry.layouts.FIXED_SPREAD);
      });
    
      it('should not change the layout if it does not exist', function() {
        el.layout = 'layout1';
        
        expect(el.layout).to.equal(Coral.Masonry.layouts.FIXED_CENTERED);
        expect(el.getAttribute('layout')).to.equal(Coral.Masonry.layouts.FIXED_CENTERED);
      });
    });
  ...
  describe('Events', function() {
    ...
    describe('#coral-collection:add', function() {
      it('should trigger when adding an item', function(done) {
        ...
        const item = el.items.add(new Coral.Masonry.Item());
    ...
    describe('#coral-collection:remove', function() {
      it('should trigger when removing an item', function(done) {
        ...
        el.items.add(new Coral.Masonry.Item());
      });
    });
  });
  ...
  describe('Implementation Details', function() {
    ...
    describe('#resizable', function() {
      it('should resize the masonry when the container changes its width', function(done) {
        const container = helpers.build(window.__html__['Coral.Masonry.container.html']);
      ...
      it('should resize the masonry when the container becomes visible', function(done) {
        const container = helpers.build(window.__html__['Coral.Masonry.hidden-container.html']);
        const el = container.querySelector('coral-masonry');
