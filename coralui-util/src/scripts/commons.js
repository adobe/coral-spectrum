...
/**
 ...
 @param {Coral.commons~transitionEndCallback} onTransitionEndCallback
 The callback to execute.
 */
commons.transitionEnd = (element, onTransitionEndCallback) => {
  let propertyName;
...
/**
 ...
 @param {Coral.commons~readyCallback} onDefined
 The callback to call when all components are ready.
 */
commons.ready = (element, onDefined) => {
  let root = element;
 ...
 */
commons.setSubProperty = (root, path, obj) => {
...
/**
 ...
 @throws Will throw an error if the path is not present on the object.
 */
commons.getSubProperty = (root, path) => {
  const nsParts = path.split('.');
  let curObj = root;
 ...
 */
commons.mixin = (target, mixins, options) => {
 ...
 */
commons.getUID = () => `coral-id-${nextID++}`;
...
ResizeEventTrigger.prototype._addTriggerElement = (element, listenerFunction) => {
...
ResizeEventTrigger.prototype._removeTriggerElement = (element) => {
  if (!element._resizeTriggerElement) {
    return;
  }
...
ResizeEventTrigger.prototype._fireResizeListeners = (event) => {
 ...
 */
// eslint-disable-next-line func-names
ResizeEventTrigger.prototype.addResizeListener = function(element, onResize) {
  if (!element) {
    return;
  }
 ...
 */
// eslint-disable-next-line func-names
ResizeEventTrigger.prototype.removeResizeListener = function(element, onResize) {
  if (!element) {
    return;
  }
...
/**
 Bind static methods
 */
const resizeEvent = new ResizeEventTrigger();
