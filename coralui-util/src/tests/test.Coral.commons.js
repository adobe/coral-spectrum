describe('Coral.commons', function() {
  ...
  describe('#TABBABLE_ELEMENT_SELECTOR', function() {
    it('should not select items with tabIndex=-1', function() {
      ...
      div.tabIndex = -1;
      expect(div.matches(Coral.commons.TABBABLE_ELEMENT_SELECTOR)).to.equal(false, 'Div with tabIndex=-1 should not be selected');

      var button = document.createElement('button');
      expect(button.matches(Coral.commons.TABBABLE_ELEMENT_SELECTOR)).to.equal(true, 'Button should be selected');
    });
  });
});
